...
cd "$ROOT_DIR/services/synthea_runner"

set -a; source "$ROOT_DIR/.env"; set +a
: "${PROJECT_ID:?}"
: "${REGION:?}"
if [[ -z "${STORE_ID_PATH:-}" ]]; then
  # Try to construct from DATASET and STORE_ID
  if [[ -n "${DATASET:-}" && -n "${STORE_ID:-}" ]]; then
    STORE_ID_PATH="projects/${PROJECT_ID}/locations/${REGION}/datasets/${DATASET}/fhirStores/${STORE_ID}"
  else
    echo "ERROR: STORE_ID_PATH not set. Run scripts/fill_env.sh or export DATASET and STORE_ID." >&2
    exit 1
  fi
fi
...
# Build
gcloud builds submit -t "$IMAGE" . --project "$PROJECT_ID"
